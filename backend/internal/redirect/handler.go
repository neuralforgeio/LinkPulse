...
// handler.go — GET /{code}: resolve, enforce status, record, redirect
// (PRD 9.5.1–9.5.4).
...
func NewHandler(svc *Service, buf *clickbuffer.Buffer, log *slog.Logger, ipSalt, frontendOrigin string) *Handler {
    return &Handler{svc: svc, buf: buf, log: log, ipSalt: ipSalt, frontend: frontendOrigin}
}
...
func (h *Handler) Resolve(w http.ResponseWriter, r *http.Request) {
    ...
    // Password-protected links go to the password page first
    // (PRD 9.5.2 → frontend /protected/{code}; page lands next session).
