...
// with email OTP as a second factor).
...
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if res.OtpRequired {
		// Deliver the code. If the email fails (e.g., Resend free tier
		// only delivers to the account owner), fall back to logging
		// the code — email failure must NEVER block login entirely.
		if err := h.mailer.Send(r.Context(), res.User.Email,
			"Your LinkPulse login code", otpEmailHTML(res.OtpRawCode)); err != nil {
			h.log.Error("otp email send failed — falling back to server log",
				"error", err, "email", res.User.Email)
			h.log.Info("otp code (email fallback — read this to log in)",
				"email", res.User.Email, "code", res.OtpRawCode)
		} else if h.mailer.InDevMode() {
			h.log.Info("otp code (dev mode)",
				"email", res.User.Email, "code", res.OtpRawCode)
		}
